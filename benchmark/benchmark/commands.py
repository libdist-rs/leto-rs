...
class CommandMaker:
    @staticmethod
    def cleanup():
        return f"rm -rf .db-* ; rm -f *.json ; mkdir -p {PathMaker.results_path()}"
    ...
    @staticmethod
    def profile_build():
        return f"cargo build --quiet --release -p node --features=benchmark --features=consensus/microbench"
    ...
    @staticmethod
    def profile_server(key_file, id, server_file, debug=False):
        assert isinstance(key_file, str)
        assert isinstance(server_file, str)
        assert isinstance(debug, bool)
        v = "-vvvv" if debug else "-vvv"
    ...
    @staticmethod
    def profile_client(id, client_file):
        assert isinstance(id, int)
        assert isinstance(client_file, str)
        return f"flamegraph -o fl-cl{id}.svg -- node -vvv client --id {id} --config ./{client_file}"
