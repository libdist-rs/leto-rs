...
class PathMaker:
    ...
    @staticmethod
    def result_file(faults, nodes, collocate, rate, tx_size):
        return join(
    ...
    @staticmethod
    def agg_file(type, faults, nodes, collocate, rate, tx_size, max_latency=None):
        if max_latency is None:
            name = f'{type}-bench-{faults}-{nodes}-{collocate}-{rate}-{tx_size}.txt'
        else:
            name = f'{type}-{max_latency}-bench-{faults}-{nodes}-{collocate}-{rate}-{tx_size}.txt'
